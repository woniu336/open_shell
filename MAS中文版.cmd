...
if %errorlevel% NEQ 0 (
echo:
echo Null 服务未运行，脚本崩溃...
echo:
echo:
echo 访问此网站获得帮助 - %mas%troubleshoot.html
echo:
echo:
ping 127.0.0.1 -n 10
)
...
%nceline%
echo 系统版本不支持！ [当前版本 %winbuild%].
echo 仅支持Windows 7/8/8.1/10/11桌面版本和对应的服务器版本。
goto MASend
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 在系统中找不到 powershell.exe 程序.
echo 脚本中断执行...
goto MASend
)
...
echo [0] 继续运行过时的版本
echo:
call :_color %_Green% "通过键盘选择一个选项 [1,0] :"
choice /C:10 /N
if !errorlevel!==2 rem
...
echo:
echo:                       windows / office 激活选项:
echo:
echo:         [1] HWID(数字许可证) ^|  Windows           ^|  永久激活
echo:         [2] Ohook            ^|  Office            ^|  永久激活
echo:         [3] KMS38            ^|  Windows           ^|  激活到2038年
echo:         [4] 在线KMS激活      ^|  Windows / Office  ^|  180 天循环激活
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "          " %_Green% "请按键盘选择 [1,2,3,4,5,6,7,8,0] :"
...
echo:       ______________________________________________________________
echo:
echo:             [1] 更换 windows 系统版本
echo:
echo:             [2] 提取 $OEM$ 脚本到文件夹
echo:
echo:             [3] 查看激活状态 [vbs方式]
echo:
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "请按键盘选择 [1,2,3,4,0] :"
choice /C:12340 /N
set _erl=%errorlevel%
...
echo:
call :_color2 %_White% "              [R] " %_Green% "查看帮助"
echo:              [0] 返回上一级
echo:           ________________________________________________________
echo:  
call :_color2 %_White% "           " %_Green% "请按键盘选择:"
...
call :_color %Green% "$OEM$ 已经成功提取到桌面."
)
echo "%_oem%" | find /i "KMS38" 1>nul && (
echo:
echo 要切换到KMS38激活服务器Cor/Acor版本 ^(无 GUI 版本^),
echo 请查看这个页面 %mas%oem-folder
)
...
if %~z0 GEQ 200000 (
set "_exitmsg=返回"
set "_fixmsg=返回主菜单, 选择 故障排除，然后选择 修复Licensing选项来修复错误."
) else (
set "_exitmsg=退出"
set "_fixmsg=打开 MAS 文件夹, 运行 Troubleshoot 脚本 并选择 修复Licensing 选项."
)
...
%eline%
echo 不支持当前系统 [当前版本 %winbuild%].
echo 数字许可证方式激活只支持 Windows 10/11.
echo 请选择 在线KMS激活 选项进行激活.
goto dk_done
)

if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
%eline%
echo windows服务器版本不支持数字许可证方式激活.
echo 请使用 KMS38 或 在线KMS激活 选项.
goto dk_done
)
...
echo:
echo 无法运行 PowerShell . 执行中断...
echo 如果你限制了 Powershell 运行，请解除限制.
echo:
echo 通过这个页面获取帮助： %mas%troubleshoot
goto dk_done
)
...
call :dk_color %Red% "启动 [sppsvc] 服务失败, 可能会长时间没反应..."
echo:
)
...
echo:
call :dk_color2 %_White% "     " %Green% "激活检测: %winos% 已永久激活."
call :dk_color2 %_White% "     " %Gray% "当前系统不需要激活."
echo ___________________________________________________________________________________________
...
echo 查看系统信息                        [%winos% ^| %fullbuild% ^| %arch%]
...
if defined _int (
echo 检查联网状态            [已联网 %ping_f%]
) else (
set error=1
call :dk_color %Red% "检查联网状态            [未联网]"
)
...
echo 启用 Windows Script Host            [成功]
)
...
if defined updatesblock call :dk_color %Gray% "注册表中是否阻止了系统更新     [是]"

if defined applist echo: %serv_e% | find /i "wuauserv" %nul% && (
call :dk_color %Blue% "Windows更新无法运行。如果您已禁用它，请启用它."
reg query HKLM\SYSTEM\CurrentControlSet\Services\wuauserv /v WubLock %nul% && call :dk_color %Blue% "已使用Sordum Windows Update Blocker工具来阻止系统更新."
)

reg query "HKLM\SOFTWARE\Policies\Microsoft\WindowsStore" /v DisableStoreApps %nul2% | find /i "0x1" %nul% && (
call :dk_color %Gray% "注册表中是否设置了阻止存储      [是]"
)
...
call :dk_color %Red% "检查HWID的备用版本     [%altedition% 未找到激活ID]"
)
...
echo 在 %SystemRoot%\System32\spp\tokens\skus\ 中找不到所需的许可证文件
)
echo 确保您使用的脚本是最新版本.
echo %mas%
echo:
...
call :dk_color %Blue% "使用 [%altedition%] 版本产品密钥进行数字许可证(HWID)激活."
echo:
)
...
if defined altapplist call :dk_color %Red% "没有找到此密钥的激活ID."
...
call :dk_color %Red% "检查ClipUp.exe文件                [未找到，中止进程]"
call :dk_color2 %Blue% "请查看此页面以获得帮助" %_Yellow% " %mas%troubleshoot"
goto :dl_final
)
...
set error=1
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "安装 GenuineTicket.xml            [ClipSVC服务重启失败，等待中…]"
)
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "检查ClipSVC token .dat             [不存在]"
)

%_xmlexist% (
set error=1
set rebuildinfo=1
call :dk_color %Red% "安装 GenuineTicket.xml            [执行clip -v -o失败]"
)
...
call :dk_color2 %Blue% "请查看此页面以获得帮助" %_Yellow% " %mas%licensing-servers-issue"
)
)
...
call :dk_color %Red% "删除注册表                     [失败] [!_ident!]"
) || (
...
set error=1
echo:
call :dk_color %Red% "检查网络连接是否有问题                [是] %error_code%"
call :dk_color2 %Blue% "请查看此页面以获得帮助" %_Yellow% " %mas%licensing-servers-issue"
)
)
...
call :dk_color %Blue% "在编写此脚本时，此产品还不支持HWID激活."
call :dk_color %Blue% "请使用 KMS38激活 选项."
) else (
...
call :dk_color %Red% "恢复Windows区域                [失败] [%name% - %nation%]"
)
)

if %osSKU%==175 call :dk_color %Red% "%winos% 不支持非azure平台上的激活."
...
if defined serv_cor (
set error=1
call :dk_color %Red% "检查 Corrupt 服务               [%serv_cor%]"
)
...
if defined serv_csts call :dk_color %Gray% "启用被禁用的服务              [成功] [%serv_csts%]"

if defined serv_cste (
set error=1
call :dk_color %Red% "启用被禁用的服务              [失败] [%serv_cste%]"
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查启动方式                      [%safeboot_option%] " %Blue% "[当前为安全模式，请以正常模式运行.]"
)
...
set error=1
call :dk_color %Red% "检查Windows安装状态            [%imagestate%]"
echo "%imagestate%" | find /i "RESEAL" %nul% && (
set showfix=1
call :dk_color %Blue% "如果您在审计模式下运行，请切换到正常模式下运行."
)
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查PE环境                          " %Blue% "[当前为WinPE模式，请在正常模式运行.]"
)
...
if %dism_error% NEQ 0 (
call :dk_color %Red% "检查DISM                           [没有响应] [%dism_error%]"
)
...
set error=1
set showfix=1
call :dk_color %Red% "检查Eval包                  [Non-Eval license安装在Eval Windows中]"
call :dk_color %Blue% "评估版windows无法激活，这可能是由于错误的许可证安装导致的."
...
set error=1
call :dk_color %Red% "检查License文件                  [未找到] [%osedition%]"
)

if not exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-*-%osedition%-*.mum" (
set error=1
call :dk_color %Red% "检查安装包                   [未找到] [%osedition%]"
)
)
)
...
if %error_code% NEQ 0 (
set error=1
call :dk_color %Red% "检查slmgr /dlv信息                     [未响应] %error_code%"
)
...
call :dk_color %Blue% "重新运行，并选择“故障排除”并运行“修复WMI”选项."
set showfix=1
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查Rearm信息                          " %Blue% "[系统需要重启]"
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查 ClipSVC                        " %Blue% "[系统需要重启]"
)


for /f "skip=2 tokens=2*" %%a in ('reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform" /v "SkipRearm" %nul6%') do if /i %%b NEQ 0x0 (
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform" /v "SkipRearm" /t REG_DWORD /d "0" /f %nul%
call :dk_color %Red% "检查 SkipRearm                      [未找到默认值0。改为0]"
...
call :dk_color %Red% "检查SPP注册表项               [发现不正确的ModuleId]"
call :dk_color %Blue% "可能由Gaming Spoofers引起. 查看帮助: %mas%troubleshoot"
set error=1
set showfix=1
)
...
%psc% "!d!" %nul%
call :dk_color %Gray% "检查SPP令牌文件夹               [没有找到，现在创建] [%tokenstore%\]"
)
...
set error=1
set showfix=1
call :dk_color %Red% "检查SPP权限                [发现错误]"
call :dk_color %Blue% "%_fixmsg%"
)
...
set showfix=1
call :dk_color %Blue% "%_fixmsg%"
if not defined permerror call :dk_color %Blue% "如果激活仍然失败，，则返回主菜单，选择“故障排除”并运行“修复WPA注册表”选项。"
)

if not defined showfix if defined wpaerror (
set showfix=1
call :dk_color %Blue% "如果激活失败，则返回主菜单，选择“故障排除”并运行“修复WPA注册表”选项。"
)
...
if %~z0 GEQ 200000 (
set "_exitmsg=返回"
set "_fixmsg=回到主菜单，选择“故障排除”并运行“修复Licensing”选项。"
) else (
set "_exitmsg=退出"
set "_fixmsg=在MAS文件夹中，运行“Troubleshoot”脚本并选择“修复Licensing”选项。"
)
...
echo Ohook激活只支持Windows 8及更高版本或相应的服务器版本.
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "请按键盘选择 [1,2,3,0]"
choice /C:1230 /N
set _el=!errorlevel!
if !_el!==4  exit /b
...
call :dk_color %Red% "启动 [sppsvc] 服务失败, 可能会长时间无响应..."
echo:
)
...
echo:
call :dk_color %Red% "检查是否安装了不支持的Office版本     [ %o14msi%%o14c2r%%o16uwp%]"
)
...
call :dk_color %Red% "检查 ClickToRun 服务             [未找到，找到Office 16.0]"
set o16c2r=
set error=1
)
...
call :dk_color %Red% "检查 ClickToRun 服务             [未找到，找到Office 15.0]"
set o15c2r=
set error=1
)
...
echo:
echo 如果你只安装了Office仪表盘应用程序，你需要安装完整的Office版本.
)
echo:
call :dk_color %Blue% "从下面的URL下载并安装Office，然后再试一次."
echo:
echo %mas%genuine-installation-media.html
goto dk_done
)
...
call :dk_color %Gray% "检查多个Office安装        [发现。最好只安装一个版本]"
)
...
call :dk_color %Red% "检查已安装产品             [没有找到产品id，中止激活...]"
set error=1
goto :starto16c2r
)
...
if not defined _sid (
call :dk_color %Red% "检查用户帐号SID              [未找到]"
)
...
if defined sub_next echo 移除Office vNext Block             [成功]
...
if %upk_result%==1 echo 卸载Other/Grace密钥           [成功]
if %upk_result%==2 call :dk_color %Red% "卸载Other/Grace密钥           [失败]"
...
call :dk_color %Gray% "安装丢失的License文件        [Office %oVer%.0 %_prod%] [成功]"
exit /b
)
...
call :oh_actids
echo "!oapplist!" | find /i "!_actid!" %nul1% && (
call :dk_color %Gray% "安装丢失的License文件        [Office %oVer%.0 %_prod%] [使用/ilc方法成功]"
) || (
set error=1
call :dk_color %Red% "安装丢失的License文件        [Office %oVer%.0 %_prod%] [失败]"
)
...
call :dk_color %Blue% "如果仍然没有解决，重启系统，再试一次。"
)
...
set error=1
call :dk_color %Red% "在脚本中检查产品              [Office %oVer%.0 !_prod! 在脚本中没有找到]"
call :dk_color %Blue% "确保您使用的是最新的MAS脚本."
)
)
...
%eline%
echo 检测到不支持的操作系统版本 [%winbuild%].
echo KMS38激活 仅支持Windows 10/11/服务器版本、build 14393及更高版本。
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "请按键盘选择 [1,2,0]"
choice /C:120 /N
set _el=!errorlevel!
if !_el!==3  exit /b
...
echo ___________________________________________________________________________________________
echo:
call :dk_color2 %_White% "     " %Green% "激活检查: %winos% 已经永久激活啦."
...
echo 无法激活评估版服务器系统，请将其转换为完整的服务器操作系统.
echo:
echo 返回主菜单，选择“附加功能”并使用“更改版本”选项.
) else (
echo 无法激活评估版本. 
...
%eline%
echo clipup.exe在Server Cor/Acor [No GUI]版本中不存在.
echo 这是KMS38激活所必需的.
echo 查看下面的网页了解如何激活.
echo %mas%kms38.html
goto dk_done
...
call :dk_color %Blue% "要使用KMS38激活IotEnterpriseS版本，系统必须更新到 build 19044.2788 或更高版本."
)
)

if not defined key if defined notfoundaltactID (
call :dk_color %Red% "检查KMS38的备用版本    [%altedition% 未找到激活ID]"
)
...
echo:
if defined changekey (
call :dk_color %Blue% "将使用 [%altedition%] 版本产品密钥进行KMS38激活."
echo:
)
...
call echo 检查已安装产品密钥          [Partial Key - %%_partial%%] [Volume:GVLK]
)
...
if !error_code! EQU 0 (
call :dk_refresh
echo 安装KMS客户端设置密钥         [%key%] [成功]
) else (
call :dk_color %Red% "安装KMS客户端设置密钥         [%key%] [失败] !error_code!"
if not defined error (
call :dk_color %Blue% "%_fixmsg%"
set showfix=1
)
...
if not defined app (
call :dk_color %Red% "检查已安装的GVLK激活ID   [未找到] 中止执行..."
call :dk_color2 %Blue% "请查看此页面以获得帮助" %_Yellow% " %mas%troubleshoot"
goto :dk_done
)
...
echo 添加指定KMS主机                [LocalHost 127.0.0.2] [成功]
) else (
call :dk_color %Red% "添加指定KMS主机                [LocalHost 127.0.0.2] [失败]"
)
...
echo:
if exist "!_clipup!" (
echo 复制clipup.exe文件到              [%systemroot%\System32\] [成功]
) else (
call :dk_color %Red% "复制clipup.exe文件到              [%systemroot%\System32\] [失败] 中止执行..."
goto :k_final
)
)
...
set error=1
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "安装 GenuineTicket.xml            [重启 ClipSVC Service 服务失败, 请稍候...]"
)
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "安装 GenuineTicket.xml            [使用 clipup -v -o 失败]"
)
...
%nul% reg delete "HKU\S-1-5-20\%specific_kms%" /f
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Red% "移除添加的特定KMS主机    [失败]"
) || (
echo 移除添加的特定KMS主机    [成功]
)
)
...
%nul% reg delete "HKLM\%specific_kms%" /f
%nul% reg query "HKLM\%specific_kms%" && (
echo 保护KMS38免受KMS攻击                  [成功] [锁定注册表项]
) || (
call :dk_color %Red% "保护KMS38免受KMS攻击                  [锁定注册表项失败]"
)
)
...
if defined a_cor (
if exist "%_clipup%" (
call :dk_color %Red% "删除复制的clipup.exe文件         [失败]"
) else (
echo 删除复制的clipup.exe文件         [成功]
)
)

for %%# in (175 407) do if %osSKU%==%%# (
call :dk_color %Red% "%winos% 不支持在非azure平台上激活。"
)
...
echo:
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Red% "移除指定KMS主机              [失败]"
) || (
echo 移除指定KMS主机              [成功]
)
...
call :dk_color %Green% "%winos% 已激活到 !xpr!"
exit /b
...
for %%# in (wmic.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 无法在系统中找到wmic.exe。
if %winbuild% GEQ 22621 echo 确保在可选特性中启用了WMIC。
goto Done
)
...
echo WMI在系统中没有响应。
echo:
echo 返回主菜单，选择“故障排除”并运行“修复WMI”选项。
goto Done
)
...
echo.              [3] 激活 - 全部
echo.
if defined _tskinstalled call :_color2 %_White% "              [4] 添加循环激活任务计划      " %_Green% "[已安装]"
if defined _oldtsk       call :_color2 %_White% "              [4] 添加循环激活任务计划      " %_Red% "[已存在]"
if not defined _tskinstalled if not defined _oldtsk echo.              [4] 添加循环激活任务计划
echo.              [5] 卸载
...
echo.       ______________________________________________________________
echo.
call :_color2 %_White% "           " %_Green% "请按键盘选择 [1,2,3,4,5,6,7,0]"
choice /C:12345670 /N
set _el=%errorlevel%
...
if defined _unattended (
echo.
call :_color %_Red% "互联网没有连接，但仍然会继续执行."
) else (
%eline%
echo 没有连接互联网.
echo:
...
set "nKMS=不支持KMS激活..."
set "nEval=无法激活评估版本，请安装完整的Windows操作系统."
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-*EvalEdition~*.mum" set _eval=1
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*EvalEdition~*.mum" set "nEval=无法激活评估版服务器，请转换为完整的服务器操作系统."
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*EvalCorEdition~*.mum" set _eval=1&set "nEval=无法激活评估版服务器，请转换为完整的服务器操作系统."
set "_C16R="
...
if %ActOffice% EQU 0 (echo.&echo 没有选择激活Office...)
...
if %WinVL% EQU 0 (
if %ActWindows% EQU 0 (
  echo.&echo Windows激活是关闭的...
  ) else (
  if %SSppHook% EQU 0 (
    echo.&echo %_winos% %nKMS%
    if defined _eval echo %nEval%
    ) else (
    echo.&echo 检查Windows的KMS激活ID失败. &call :CheckWS
    exit /b
    )
  )
)
...
if %W1nd0ws% EQU 1 if %ActWindows% EQU 0 (echo.&echo 没有选择激活Windows...)
...
if %winbuild% GEQ 9200 (
  if %OffUWP% EQU 0 (echo.&echo 检测到未安装Office 2013-2021产品...) else (echo.&echo %_mOuwp%)
  exit /b
  )
if %winbuild% LSS 9200 (if %loc_off14% EQU 0 (echo.&echo 检测到未安装 Office %aword% 产品...&exit /b))
)
...
if %OsppHook% EQU 0 (echo.&echo 检测到未安装Office %aword% ...&exit /b)
if %winbuild% GEQ 9200 if %loc_off14% EQU 0 (echo.&echo 检测到未安装 Office %aword% ...&exit /b)
set err_offsvc=0
net start osppsvc /y %_Nul3% || (
sc start osppsvc %_Nul3%
if !errorlevel! EQU 1053 set err_offsvc=1
)
if %err_offsvc% EQU 1 (echo.&echo 错误: 没有运行 osppsvc 服务...&exit /b)
...
set "_key="
set "_qr=%_zz7% %spp% %_zz2% %_zz5%ID='%app%'%_zz6% %_zz3% Name %_zz8%"
if %ESU_ADD% EQU 0 for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 安装序列号: %%x
if %ESU_ADD% EQU 1 for /f "tokens=2 delims==f" %%x in ('%_qr%') do echo 安装序列号: %%x
set ESU_ADD=0
call :keys %app%
if "%_key%"=="" (echo 没有找到关联的KMS客户端密钥&exit /b)
...
echo 产品激活成功，但没有延长有效期。
if %OSType% EQU Win7 echo 这可能与KB4487266中描述的错误有关
exit /b
)
set Act_OK=0
...
if "%_ProductIds%"=="" (
if %_Office15% EQU 0 (echo 错误: 未检测到Office C2R产品id&goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_LicensesPath%\ProPlus*.xrm-ms" (
if %_Office15% EQU 0 (echo 错误:未检测到Office C2R license文件&goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_Integrator%" (
if %_Office15% EQU 0 (echo 错误:未检测到Office C2R许可证集成商&goto :%_fC2R%) else (goto :Reg15istry)
)
...
if "%_Product15Ids%"=="" (
if %_Office16% EQU 0 (echo 错误:未检测到Office 2013 C2R产品id&goto :%_fC2R%) else (goto :CheckC2R)
)
if not exist "%_Licenses15Path%\ProPlus*.xrm-ms" (
if %_Office16% EQU 0 (echo 错误:未检测到Office 2013 C2R许可证文件&goto :%_fC2R%) else (goto :CheckC2R)
)
if %winbuild% LSS 9200 if not exist "%_OSPP15VBS%" (
if %_Office16% EQU 0 (echo 错误:Office 2013 C2R授权工具OSPP.VBS不存在&goto :%_fC2R%) else (goto :CheckC2R)
)
...
echo.
echo Microsoft Office使用vNext许可证激活。
)
if %sub_proj% EQU 1 (
  for %%a in (%_PrjSKU%) do set _%%a=0
echo.
echo Microsoft Project使用vNext许可证激活。
)
if %sub_vsio% EQU 1 (
  for %%a in (%_VisSKU%) do set _%%a=0
echo.
echo Microsoft Visio是通过vNext许可证激活的。
)
...
echo 将不存在的IP地址10.0.0.10保留为KMS Server。
) else (
...
if not [%Act_OK%]==[1] (
echo.
echo 如有问题，请访问 https://mass%-%grave.dev/troubleshoot
)
...
call :_color %_Red% "没有连接互联网."
exit /b
)

if [%ERRORCODE%]==[-1073418124] (
echo 检查1688端口连接，可能需要一些时间...
echo.
...
call :_color %Red% "1688端口在您的互联网连接中被阻塞."
echo.
echo 可能原因:  可能已连接受限制的互联网 [办公网络/学校网络]
echo           或防火墙阻止了连接。
echo.
echo 解决方案: 要么使用另一个Internet连接，要么使用离线KMS
echo           https://github.com/abbodi1406/KMS_VL_ALL_AIO
) else (
echo 1688端口连接测试通过。
echo.
echo 确保系统文件没有被防火墙阻止。
echo 如果问题仍然存在，请尝试脱机KMS
echo https://github.com/abbodi1406/KMS_VL_ALL_AIO
)
echo.
)

echo 这些情况都并非KMS服务器问题。
exit /b
...
echo.
echo 信息:
echo 如果连接互联网，将每周更新一次激活。
echo 它只会更新已安装的KMS许可证, 它不会将任何许可证转换为KMS。
echo __________________________________________________________________________________________
echo.
if defined ActTask (
call :_color %Green% "续订和激活任务已成功创建。"
) else (
call :_color %Green% "创建更新任务成功。"
)
echo.
call :_color %Gray% "确保至少运行了一次激活选项。"
echo __________________________________________________________________________________________
)
...
(
echo   此脚本的用途是使用在线KMS更新Windows/Office KMS许可证.
echo:
echo   如果创建了更新/激活计划任务，则将存在以下内容,
echo:
echo   - 计划任务
echo     Activation-Renewal    [更新 / 每周]
echo     Activation-Run_Once   [激活任务-激活后自动删除]
echo     只有当系统连接到Internet时，定时任务才能运行。
echo:
echo   - 文件
...
echo:
echo   在线KMS激活脚本是“Microsoft_Activation_Scripts”[MAS]项目的一部分。
echo:   
...
if defined run_once (
echo.
echo 删除计划任务Activation-Run_Once
schtasks /delete /tn Activation-Run_Once /f %nul%
)
...
if [%1]==[act_win] if %_kms38% EQU 1 (
echo 已使用KMS38激活Windows
exit /b
)

if %errorcode% EQU 12345 (
echo 产品激活失败
echo 由于受限制或没有互联网，无法测试KMS服务器
set _tserror=1
exit /b
)
...
echo 产品激活失败: 0xC004F035
if [%1]==[act_win] if %winbuild% LSS 9200 echo 由于不合格的OEM BIOS, 无法在此计算机上使用kms激活Windows 7
exit /b
)
...
echo 产品激活成功，但未能延长剩余时间。
if [%1]==[act_win] if %winbuild% LSS 9200 echo 这可能与KB4487266中描述的错误有关
set _tserror=1
exit /b
)
...
cscript //nologo slmgr.vbs /dli || (echo 执行 slmgr.vbs 错误&del /f /q slmgr.vbs&popd&goto :casVend)
...
echo %line2%
echo.
powershell "write-host -back 'Black' -fore 'Yellow' 'Office已使用Ohook永久激活.'; write-host -back 'Black' -fore 'Yellow' '您可以忽略下面显示的Office激活状态.'"
echo.
)
...
echo %line2%
if %_sO15vbs% EQU 0 (
echo ***              Office 2016 %_bit%-bit 激活状态               ***
) else (
...
echo.
echo %line2%
echo ***             Office 2013 %_bit%-bit 激活状态             ***
echo %line2%
cscript //nologo "!office!\ospp.vbs" /dstatus
)
...
echo.
echo %line2%
echo ***             Office 2010 %_bit%-bit 激活状态             ***
echo %line2%
cscript //nologo "!office!\ospp.vbs" /dstatus
)
...
if %GracePeriodRemaining% EQU 0 (
  if %winID% EQU 1 (set "ExpireMsg=这台电脑已永久激活.") else (set "ExpireMsg=该产品已永久激活.")
  ) else (
  set "LicenseMsg=%_mTag% 激活期限: %GracePeriodRemaining% 分钟 (%_gpr% 天)"
  if defined _xpr set "ExpireMsg=%_mTag% 激活将过期 %_xpr%"
  )
)
...
%eline%
echo 检测到不支持的操作系统版本。
echo 此命令仅支持Windows 8/8.1/10/11及其对应的服务器版本。
goto :at_back
)
...
echo %line%
echo:
echo      Dism使用Windows Update来提供修复损坏所需的文件。
echo      这将需要5-15分钟或更长时间..
echo %line%
echo:
echo      提示:
echo:
call :_color2 %_White% "     - " %Gray% "确保已连接互联网."
...
echo:
call :_color %Gray% "CBS和DISM日志被拷贝到desktop的“AT_Logs”文件夹中。"
goto :at_back
...
echo %line%
echo:    
echo      系统文件检查程序将修复丢失或损坏的系统文件。
echo      这将需要10-15分钟或更长时间..
echo:
echo      如果SFC无法修复某些问题，则再次运行该命令，看看下次是否能够修复。
echo      有时可能需要运行 sfc /scannow 命令3次，
echo      每次运行后需要重新启动PC后才能完全修复它所能修复的一切。
echo:   
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] 返回 : "
...
echo:
call :_color %Gray% "CBS日志被复制到dekstop的AT_Logs文件夹中."
goto :at_back
...
echo:
echo       - 这个选项将,
echo            - 停用Windows和Office，您可能需要重新激活。
echo              如果Windows是用主板/ OEM /数字许可证激活的，请不要担心
echo:
echo            - 清除ClipSVC、Office vNext、SPP和OSPP license
echo            - 修复令牌文件夹和注册表的SPP权限
echo            - 触发Office的修复选项。
echo:
call :_color2 %_White% "      - " %Red% "确保只在必要的时候使用。"
echo:
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] 返回 : "
...
echo %line%
echo:
call :_color %Blue% "重建ClipSVC许可证"
echo:

if %winbuild% LSS 10240 (
echo ClipSVC许可证重建仅支持win10 /11和相应服务器版本.
echo 跳过...
goto :cleanvnext
)
...
echo:
echo 重新启动[widsvc licensemmanager]服务...
...
echo %line%
echo:
call :_color %Blue% "清除 Office vNext License"
echo:
...
if not defined regfound echo 找不到- Office vNext注册表项
...
echo %line%
echo:
call :_color %Blue% "重新构建SPP许可令牌"
echo:

call :scandat check

if not defined token (
call :_color %Red% "找不到tokens.dat文件。"
) else (
...
for /f "delims=" %%a in ('%psc% "$f=[io.file]::ReadAllText('!_batp!') -split ':wpatest\:.*';iex ($f[1]);" %nul6%') do (set wpainfo=%%a)
echo "%wpainfo%" | find /i "Error Found" %nul% && (
call :_color %Red% "WPA注册表错误: %wpainfo%"
) || (
echo WPA注册表计数: %wpainfo%
)
...
echo:
if not defined token (
call :_color %Red% "未能重建tokens.dat文件。"
) else (
echo tokens.dat文件已成功重建.
)
...
echo %line%
echo:
call :_color %Blue% "重建OSPP许可令牌"
echo:

sc qc osppsvc %nul% || (
echo 未安装基于OSPP的Office软件
echo 跳过重建OSPP令牌...
goto :repairoffice
)
...
echo %line%
echo:
call :_color %Blue% "修复 Office Licenses"
echo:
...
if /i "%arch%"=="ARM64" (
echo:
echo 找到ARM64 Windows.
echo 您需要在Office的Windows设置中使用修复选项.
echo:
...
if defined uwp16 (
echo:
echo 注意: 不修复Office 16.0 UWP. 
echo       您需要在Windows设置中使用重置选项.
echo ________________________________________________________________
echo:
start ms-settings:appsfeatures
)
...
if defined c2r14 (
echo:
echo 注意: 跳过修复Office 14.0 C2R
echo       您需要在Windows设置中使用修复选项.
echo ________________________________________________________________
echo:
start appwiz.cpl
)
...
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
%eline%
echo 不建议在Windows Server上重新构建WMI，中止执行...
goto :at_back
)

for %%# in (wmic.exe) do @if "%%~$PATH:#"=="" (
%eline%
echo 系统中找不到Wmic.exe文件，中止执行...
goto :at_back
)
...
call :_color %Red% "[失败]"
echo:
call :_color %Blue% "建议选择[重启]选项，然后再次执行修复WMI选项。"
echo %line%
echo:
...
call :_color %Red% "[没有响应]"
echo:
echo 运行[Dism RestoreHealth]和[SFC Scannow]选项来检查是否有错误.
) else (
call :_color %Green% "[工作正常]"
)
...
echo:
echo Error code: %errorlevel%
call :dk_color %Red% "启动[sppsvc]服务失败，可能需要较长时间..."
echo:
)
...
DISM /English /Online /Get-CurrentEdition %nul%
cmd /c exit /b !errorlevel!
echo DISM命令失败 [错误代码 - 0x!=ExitCode!]
echo 未能检测到OS版本，中止执行...
echo:
echo 请查看此页面以获得帮助. %mas%troubleshoot
goto ced_done
...
%line%
echo:
if defined dismnotworking call :dk_color %Red% "DISM.exe没有响应."
...
%line%
echo:
call :dk_color %Gray% "您可以将 [%osedition%] [%winbuild%] 更改为以下其中一种。"
if defined dismnotworking (
call :dk_color %_Yellow% "注意- DISM.exe没有响应。"
...
echo [0]  %_exitmsg%
echo:
call :dk_color %_Green% "在键盘上输入选项，然后按“Enter”键:"
set /p inpt=
...
echo 将当前版本 [%osedition%] %winbuild% 更换为 [%targetedition%]
echo:

if %_dismapi%==1 (
call :dk_color %Green% "注意-"
echo:
echo  - 保存工作后再继续，系统将自动重启。
echo:
echo  - 一旦版本更改，您将需要使用HWID选项重新激活.
%line%
echo:
...
call :dk_color %Red% "[未成功] [错误代码: 0x!=ExitCode!]"
echo 请查看此页面以获得帮助. %mas%troubleshoot
)
)
...
echo:
if defined dismnotworking call :dk_color %_Yellow% "注意- DISM.exe没有响应。"
echo 将 [%osedition%] %winbuild% 更改为 [%targetedition%]
echo:
call :dk_color %Blue% "重要提醒-在继续之前请先保存您的工作，系统将自动重新启动。"
echo:
...
echo:
if defined dismnotworking call :dk_color %_Yellow% "注意- DISM.exe没有响应。"
echo 将当前的 [%osedition%] %winbuild% 更改为 [%targetedition%]
echo:
echo 使用 %_chan% 值
...
call :dk_color %Blue% "在这个阶段，您必须重新启动系统。"
echo 查看帮助: %mas%troubleshoot
...
if($null -eq $editionXml) {
    Write-Warning '找不到版本特定的XML设置，在没有它的情况下继续...'
}

Write-Host '启动升级进程。这可能需要一段时间...'
