...
set_cron_job() {
    ...
    # 添加执行权限
    if ! sudo chmod +x /root/logcheck/run_log_check_and_ban.sh; then
        echo "无法添加执行权限，请检查您的sudo权限。"
        echo "按 Enter 键继续..."
        read
        return
    fi
    ...
    # 确保正确的所有权
    if ! sudo chown root:root /root/logcheck/run_log_check_and_ban.sh; then
        echo "无法更改文件所有权，请检查您的sudo权限。"
        echo "按 Enter 键继续..."
        read
        return
    fi
