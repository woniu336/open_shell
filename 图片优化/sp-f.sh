...
source_dir="/c/Users/Administrator/Desktop/vod"
...
process_image() {
    ...
    # 获取图片尺寸
    dimensions=$("$magick_path" "$file" -format "%wx%h" info:)
    width=$(echo $dimensions | cut -d'x' -f1)
    height=$(echo $dimensions | cut -d'x' -f2)
    
    # 调整大小和优化
    if [ $width -gt 270 ] || [ $height -gt 400 ]; then
        resize_option="-resize 270x400>"
    else
        resize_option=""
    fi
    ...
    if ! "$magick_path" "$file" $resize_option -strip -interlace Plane -quality 85% "$temp_file"; then
        log_message "错误：无法优化图片: $file"
